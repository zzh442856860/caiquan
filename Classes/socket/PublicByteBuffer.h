/****************************************************************************
 *
 * PublicByteBuffer Class
*
 */

#ifndef _Public_BYTEBUFFER_H
#define _Public_BYTEBUFFER_H

#include "Common.h"
//#include "Vector3.h"

class  PublicByteBuffer
{
...
#define DEFINE_BUFFER_WRITE_OPERATOR(type) void operator << (const type src) { Write<type>(src); }
	 ...
	 */
#define DEFINE_FAST_READ_OPERATOR(type, size) PublicByteBuffer& operator >> (type& dest) { if(m_readPos + size > m_writePos) { dest = (type)0; return *this; } else { dest = *(type*)&m_buffer[m_readPos]; m_readPos += size; return *this; } }
#define DEFINE_FAST_WRITE_OPERATOR(type, size) PublicByteBuffer& operator << (const type src) { if(m_writePos + size > m_buffersize) { reserve(m_buffersize + DEFAULT_INCREASE_SIZE); } *(type*)&m_buffer[m_writePos] = src; m_writePos += size; return *this; }
	...
	*/
	...
	PublicByteBuffer& operator >> (bool & dst) { dst = (Read<char>() > 0 ? true : false); return *this; }

	/** string (null-terminated) operators
	 */
	PublicByteBuffer& operator << (const std::string & value) { EnsureBufferSize(value.length() + 1); memcpy(&m_buffer[m_writePos], value.c_str(), value.length()+1); m_writePos += (value.length() + 1); return *this; }
	PublicByteBuffer& operator >> (std::string & dest)
	{
		dest.clear();
		char c;
		for(;;)
...
template <typename T> PublicByteBuffer &operator<<(PublicByteBuffer &b, const std::vector<T> & v)
{
	b << (uint32)v.size();
	for (typename std::vector<T>::const_iterator i = v.begin(); i != v.end(); i++) {
		b << *i;
...
template <typename T> PublicByteBuffer &operator>>(PublicByteBuffer &b, std::vector<T> &v)
{
	uint32 vsize;
	b >> vsize;
	v.clear();
...
template <typename T> PublicByteBuffer &operator<<(PublicByteBuffer &b, const std::list<T> & v)
{
	b << (uint32)v.size();
	for (typename std::list<T>::const_iterator i = v.begin(); i != v.end(); i++) {
		b << *i;
...
template <typename T> PublicByteBuffer &operator>>(PublicByteBuffer &b, std::list<T> &v)
{
	uint32 vsize;
	b >> vsize;
	v.clear();
...
template <typename K, typename V> PublicByteBuffer &operator<<(PublicByteBuffer &b, const std::map<K, V> &m)
{
	b << (uint32)m.size();
	for (typename std::map<K, V>::const_iterator i = m.begin(); i != m.end(); i++) {
		b << i->first << i->second;
...
template <typename K, typename V> PublicByteBuffer &operator>>(PublicByteBuffer &b, std::map<K, V> &m)
{
	uint32 msize;
	b >> msize;
	m.clear();
