...
WGSERVER_INLINE static void swap16(uint16* p) { *p = bswap_16((uint16_t)*p); }
WGSERVER_INLINE static void swap32(uint32* p) { *p = bswap_32((uint32_t)*p); }
WGSERVER_INLINE static void swap64(uint64* p) { *p = bswap_64((uint64_t)*p);; }

WGSERVER_INLINE static float swapfloat(float p)
{
...
WGSERVER_INLINE static uint64 swap64(uint64 p)  { return bswap_64((uint64_t)p); }

WGSERVER_INLINE static void swap16(int16* p) { *p = bswap_16((uint16_t)*p); }
WGSERVER_INLINE static void swap32(int32* p) { *p = bswap_32((uint32_t)*p); }
WGSERVER_INLINE static void swap64(int64* p) { *p = bswap_64((uint64_t)*p); }

WGSERVER_INLINE static int16 swap16(int16 p) { return bswap_16((uint16_t)p); }
WGSERVER_INLINE static int32 swap32(int32 p) { return bswap_32((uint32_t)p); }
WGSERVER_INLINE static int64 swap64(int64 p)  { return bswap_64((uint64_t)p); }
...
{	
	...
	return ((tv.tv_sec - system_start_time_t) * 1000) + (tv.tv_usec / 1000);
#endif
}
